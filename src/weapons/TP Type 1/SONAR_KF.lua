...
function onTick()
    ...
    if sonar_on then
        ...
        if lock_on then
            if not(isInit) then
                --初期値
                azimuth = {{target_x},{0}}
                elevation = {{target_z},{0}}
                isInit = true
            else
                local u,z={},{}
    
                --カルマンフィルタ
                sigmaBearing = (0.002)^2/12
                R = {{sigmaBearing}}
        ...
        --lock_on_xの更新にカルマンフィルタ適用後の値を追加する予定
    ...
    --出力
    OUN(1, azimuth[1][1])       --水平角度
    OUN(2, elevation[1][1])     --垂直角度
    OUN(3, azimuth[2][1])       --水平回転速度
    OUN(4, elevation[2][1])     --垂直回転速度

    --グラフ描画用
    table.insert(rawtable, target_x)
    table.insert(filtertable, azimuth[1][1])
    table.insert(speedtable, azimuth[2][1])
...
function onDraw()
    ...
    for i = 1, #rawtable do
        ...
        glaph = math.floor(-speedtable[i]*(h/2)*50 + h/2)
