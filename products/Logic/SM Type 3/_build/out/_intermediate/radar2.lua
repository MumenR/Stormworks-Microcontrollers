...
function onTick()
    ...
    if radar_on then
        ...
        for ID, DATA in pairs(data) do
            ...
            t_max = clamp(150*distance/2000 + 30, 30, 300)
        ...
        --目標消失でロックオン解除
        if data[lock_ID] == nil then
            lock_on = false
        end

        --標的との距離が一定以下でロックオン
        if min_ID ~= 0 then
            local distance = distance3(Px, Pz, Py, data[min_ID].predict.x.est, data[min_ID].predict.y.est, data[min_ID].predict.z.est)
            if distance < 300 and not lock_on then
                lock_ID = min_ID
                lock_on = true
            end
        end
        ...
        if min_ID ~= 0 then
            ...
            OUN(1, data[ID].predict.x.est)
            OUN(2, data[ID].predict.y.est)
            OUN(3, data[ID].predict.z.est)
            OUN(4, data[ID].predict.x.a)
            OUN(5, data[ID].predict.y.a)
            OUN(6, data[ID].predict.z.a)
            OUN(7, 1)
        end
    end
end