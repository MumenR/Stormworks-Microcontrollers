...
function onDraw()
    ...
    --速度ベクトル
    if show_VVS and math.abs(gnd_speed/spd_unit) > 0.05 then
        Lx, Ly, Lz = World2Local(Vx, Vz, Vy, 0, 0, 0, -seat_y, seat_x, 0)
        x1, y1, drawable1 = Local2Display(Lx, Ly, Lz)
        if drawable1 then
    ...
    --速度ベクトル(ヘリ用)
    if show_VVS_heli and math.abs(gnd_speed/spd_unit) > 0.05 and not (hide_VVS_heli and gnd_speed > 30) then
        Lx, Ly, Lz = World2Local(0, 1, 0, 0, 0, 0, -seat_y, seat_x, 0)
        x1, y1, drawable1 = Local2Display(Lx, Ly, Lz)
        x2, y2 = clamp(Vx, -30, 30), clamp(-Vz, -30, 30)
        if drawable1 then
    ...
    if show_bank then
        --バンク角計算
        Wx, Wy, Wz = Local2World(1, 0, 0, 0, 0, 0, Ex, Ey, Ez)
        Wx, Wy, Wz = Local2World(Wx, Wy, Wz, 0, 0, 0, 0, compass, 0)
        bank = math.acos(Wx)
